                ...
                <form action="/crypto/{{$crypto['id']}}/buy" method="get">
                    ...
                    <select name="account" id="account" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        ...
                        @foreach ($accounts as $account)
                            <option value="{{$account['account_number']}}">Account: {{$account['account_number']}} | {{number_format($account['balance'] / 100,2)}} {{$account['currency']}}</option>
                        @endforeach
                    </select>

                    <label for="amount" class="block my-2 text-center text-sm font-medium text-gray-900 dark:text-white">Amount</label>
                    <input type="number" step="0.0000000001" name="amount" id="amount" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                <form action="/crypto/{{$crypto['id']}}/sell" method="get">
                    ...
                    <select name="account" id="account" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        ...
                        @foreach ($cryptoAccounts as $cryptoAccount)
                            <option value="{{$cryptoAccount['account_number']}}">Account: {{$cryptoAccount['account_number']}} | {{number_format($cryptoAccount['balance'] / 100, 2)}} {{$cryptoAccount['currency']}}</option>
                        @endforeach
                    </select>
